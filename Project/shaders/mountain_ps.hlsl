...
float4 getcolor(OutputVertex input)
{
    float ratio = saturate((input.worldPosition.y) / 10);
    
    float4 peakcolor = float4(50.0/255, 76.0/255, 89.0/255, 1);
    float4 basecolor = float4(8.0 / 255, 74.0 / 255, 110.0 / 255, 1);
    
    float4 color = lerp(basecolor, peakcolor, ratio);
