...
float4 computeDirectionalLight(Light dlight, OutputVertex input)
{
    ...
    // Texture color
    float4 color = normalMap.Sample(diffuseSampler, input.tex);
    ...
    color = saturate(float4((ambientColor + diffuseColor + specularColor) * color.rgb, 1));
...
float4 computeSpotLight(Light slight, OutputVertex input)
{
    ...
    // Texture color
    float4 color = normalMap.Sample(normalSampler, input.tex);
...
{
    float4 color1 = normalMap.Sample(normalSampler, input.tex);
    float4 color2 = diffuse.Sample(diffuseSampler, input.tex);
    
    float4 color = color1 * color1.a + color2 * (1 - color1.a);
