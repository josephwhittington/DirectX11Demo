...
float4 computeDirectionalLight(Light dlight, OutputVertex input)
{
    ...
    // Emission color
    float3 emissionColor = emission.Sample(esampler, input.tex).rgb;
    
    // Combine phong components
    color = saturate(float4((ambientColor + diffuseColor + specularColor + emissionColor) * color.rgb, color.a));
...
float4 computeSpotLight(Light slight, OutputVertex input)
{
    ...
    color = saturate(float4((diffuseColor + specularColor + emissionColor) * color.rgb, color.a));
